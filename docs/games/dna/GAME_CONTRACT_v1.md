...
- Every active client uses a 1000 ms idle fallback poll and immediately re-syncs after returning online/to the foreground. Requests are serialized per device; failed writes retry with bounded backoff and remain tied to their original phase. When a shared server deadline is known, the transport schedules an additional boundary wake-up at that deadline instead of waiting for the next arbitrary idle poll. Server snapshots include revision, phase identity, timestamps, readiness and last-seen status. Older responses must not overwrite newer state.
- IDEA (20 s), VOTE (10 s), feedback, Reveal and countdown use common server deadlines. Clients estimate server time from request/response timestamps and a monotonic clock, excluding server processing time; device wall clocks do not decide timeouts. Network latency can cause small transport differences, but never independent deadlines or local phase advancement. During a complete outage, expired state is reconciled on the next server request without skipping unseen phases.
