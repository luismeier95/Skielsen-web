...
In Quick Games this solve notice keeps the original **floating inverse-surface pill** (`TEAM <COLOR> HAT GELÖST · +X`) with the team-color accent. The pill is anchored inside/over the black header chrome so it must never obscure the phase timer, hint cards, vote cards, or input controls.
...
- Quick Games uses one persisted, locked server session for the entire lobby, not separate encrypted gameplay states per browser. All human players must confirm Ready before IDEA starts. Reloads resume that session.
- Active clients use adaptive lightweight polling: READY/IDEA approximately 500 ms, VOTE approximately 300 ms, and non-interactive timed stages approximately 1000 ms, with an additional wake-up exactly at known server deadlines. Hidden/background tabs pause idle polling and re-sync immediately on return. Requests remain serialized per device; failed writes retry with bounded backoff and remain tied to their original phase.
- Authentication continuity is part of the live-game transport contract. A stored Supabase access token is refreshed automatically before expiry using its refresh token; a server-side 401 triggers one forced refresh + retry. A normal access-token expiry must never eject a player from an active Quick Game or reset local gameplay UI. Transient refresh/network failures remain in the reconnect flow instead of being reported as a permanent logout.
- Ordinary unchanged polls are a read-only fast path: one persisted game-state read, no row lock, no game-state write, no answer lookup and no content lookup. **The fast path is allowed only while the authoritative state cannot advance.** Early-complete conditions (all required ideas present, all required teams submitted, or no unsolved human actors remaining) must enter the locked transition path immediately; they may never wait for the nominal 20 s / 10 s deadline merely because the poll is unchanged. A row lock is taken only for player writes, due transitions or one-time state hydration. Equal revisions return only a compact clock heartbeat, and the browser must not re-render unchanged UI.
...
- The visible horizontal phase timer represents **actionable player time only** and therefore runs only during IDEA and VOTE. FEEDBACK's short server hold must not render as a rapidly draining timer.
- Once a human team has solved the current term, transient IDEA/VOTE/FEEDBACK changes for that same hint must not remount the spectator UI. If no unsolved human-controlled team remains, bot-only IDEA/VOTE phases are collapsed server-side in the same request; clients see the meaningful feedback/hint progression without 20 s / 10 s timer flashes.
