...
function makeQuickVoters(players,me,ownIndex){
 ...
 const bases=own.map((p,i)=>Array.from(String(p.display_name||'').trim().toLocaleUpperCase('de-DE'))[0]||String(i+1));
 const counts=new Map();bases.forEach(x=>counts.set(x,(counts.get(x)||0)+1));
 const seen=new Map();
 const slots=own.map((p,i)=>{
   const base=bases[i],n=(seen.get(base)||0)+1;seen.set(base,n);
   const marker=counts.get(base)>1?base+n:base;
   return {id:String(p.user_id),marker,order:i,is_me:String(p.user_id)===String(me?.user_id||'')};
 });
 if(slots.length===1)slots.push({id:'__BOT__',marker:'B',order:1,is_me:false,is_bot:true});
 return slots;
}
function currentVoterSlots(){
 return s.quickLobby&&Array.isArray(s.quickVoters)&&s.quickVoters.length
   ?s.quickVoters
   :[{id:'D',marker:'D',order:0,is_me:true},{id:'S',marker:'S',order:1,is_me:false}];
}
async function loadQuickLobby(){
 ...
 const ownIndex=Math.max(0,Math.min(3,Math.floor((Number(me.seat||1)-1)/2)));
 const ownKey=TEAM_ORDER[ownIndex]||'RED';
 ...
 TEAM_ORDER.forEach((key,index)=>{const first=bySeat.get(index*2+1),second=bySeat.get(index*2+2);TEAMS[key].members=[first?.display_name||'BOT',second?.display_name||'BOT'].map(String).map(x=>x.toUpperCase()).join(' + ')});
 s.quickVoters=makeQuickVoters(lobby.players||[],me,ownIndex);
 const teammateSeat=Number(me.seat)%2===1?Number(me.seat)+1:Number(me.seat)-1;
 s.quickHumanTeammate=Boolean(bySeat.get(teammateSeat));
 return lobby;
}
...
function setup(){clearTimers();s.screen='SETUP';setChrome('SETUP','DNA · TEAM',5);const selected=[...s.selected];page(`<section class="dna-hero"><span class="dna-kicker">TEAM QUIZ · MOBILE FIRST</span><h1>DNA.</h1><p>Drei Hinweise führen zum Begriff. Je früher euer Team löst, desto mehr Punkte.</p></section><section class="dna-card"><div class="dna-card-head"><strong>CONTENT</strong><span>${selected.length>1?'MIX · '+selected.length+' KATEGORIEN':'EINE KATEGORIE'}</span></div><div class="dna-category-grid">${CATEGORIES.map(([id,label])=>`<button class="dna-category ${s.selected.has(id)?'active':''}" data-cat="${id}" type="button">${label}</button>`).join('')}</div></section><section class="dna-card"><div class="dna-card-head"><strong>ANZAHL BEGRIFFE</strong><span>SESSION</span></div><div class="dna-choice-grid" style="--dna-choice-count:3">${[5,10,15].map(n=>`<button class="dna-choice ${s.termCount===n?'active':''}" data-terms="${n}" type="button"><strong>${n}</strong><span>BEGRIFFE</span></button>`).join('')}</div></section><section class="dna-card"><div class="dna-card-head"><strong>CONTENT MIX</strong><span>BEKANNTHEIT</span></div><div class="dna-choice-grid" style="--dna-choice-count:3">${[['CASUAL','CASUAL','Vor allem Mainstream'],['BALANCED','AUSGEWOGEN','Mainstream + Known'],['EXPERT','EXPERT','Mehr Known + Nische']].map(([id,a,b])=>`<button class="dna-choice ${s.pool===id?'active':''}" data-pool="${id}" type="button"><strong>${a}</strong><span>${b}</span></button>`).join('')}</div></section><section class="dna-summary"><div><small>CONTENT</small><strong>${selected.length===1?(CATEGORIES.find(x=>x[0]===selected[0])?.[1]||selected[0]):'MIX · '+selected.length}</strong></div><div><small>BEGRIFFE</small><strong>${s.termCount}</strong></div><div><small>POOL</small><strong>${s.pool==='BALANCED'?'AUSGEWOGEN':s.pool}</strong></div></section><div class="dna-actions"><button class="dna-btn primary dna-blocking" id="dnaSetupNext" type="button">WEITER →</button></div>`);
 ...
 $('#dnaSetupNext').addEventListener('click',saveSetup)}
async function saveSetup(){
 if(!s.quickLobby)return ready();
 const button=$('#dnaSetupNext');button.disabled=true;button.textContent='SPEICHERT …';
 try{
  s.quickLobby=await quickRpc('configure_quick_game_lobby',{p_lobby_id:s.quickLobby.lobby_id,p_categories:[...s.selected],p_term_count:s.termCount,p_pool:s.pool});
  ready();
 }catch(err){button.disabled=false;button.textContent='WEITER →';showFatal(err)}
}
function waitForQuickSetup(){
 clearTimers();s.screen='SETUP_WAIT';setChrome('SETUP','HOST WÄHLT CONTENT',5);
 page(`<section class="dna-auth"><div class="dna-auth-card"><span class="dna-kicker">QUICK GAME · ${esc(s.quickLobby.join_code)}</span><h1>SETUP LÄUFT.</h1><p>Der Host wählt Kategorien, Begriffszahl und Content-Mix. Danach geht es für alle zum Ready-Screen.</p></div></section>`);
 const poll=async()=>{try{await loadQuickLobby();if(s.quickLobby.setup_status==='READY'){applyQuickSetup();ready()}else later(poll,800)}catch(err){showFatal(err)}};later(poll,500)
}
function applyQuickSetup(){const q=s.quickLobby?.setup||{};const categories=Array.isArray(q.categories)?q.categories.filter(Boolean):[];if(categories.length)s.selected=new Set(categories);if([5,10,15].includes(Number(q.termCount)))s.termCount=Number(q.termCount);if(['CASUAL','BALANCED','EXPERT'].includes(String(q.pool)))s.pool=String(q.pool)}
function renderReady(){clearTimers();s.screen='READY';setChrome('READY',`${s.termCount} BEGRIFFE`,12);const ownActual=s.quickTeamMap?.RED||'RED';page(`<section class="dna-hero"><span class="dna-kicker">LETZTER CHECK VOR DEM SPIEL</span><h1>BEREIT?</h1></section><section class="dna-ready-rules"><div><strong>+3</strong><span>HINWEIS 1 · SEHR SCHWER</span></div><div><strong>+2 / +1</strong><span>HINWEIS 2 / 3</span></div><div><strong>−1</strong><span>FALSCHE TEAMANTWORT</span></div><div><strong>20s + 10s</strong><span>IDEE → ABSTIMMUNG</span></div></section><section class="dna-card"><div class="dna-card-head"><strong>TEAMS</strong><span>${s.quickLobby?'QUICK GAME · '+esc(s.quickLobby.join_code):'STANDALONE SIMULATION'}</span></div><div class="dna-team-list">${TEAM_ORDER.map(key=>`<div class="dna-team-row" style="--team:${TEAMS[key].color}"><i></i><div class="dna-team-ident"><strong>${TEAMS[key].name}</strong><span>${TEAMS[key].members}</span></div><b class="dna-ready-state ${key===ownActual?'':'ready'}" data-ready="${key}">${key===ownActual?'WARTET':'BEREIT'}</b></div>`).join('')}</div></section><section class="dna-card"><span class="dna-kicker">SO FUNKTIONIERT ES</span><p style="margin:10px 0 0;font-size:10px;line-height:1.5;font-weight:800">Phase 1: Jeder Spieler gibt geheim eine Idee ein. Phase 2: Nur euer Team sieht die Ideen und stimmt ab. Einigkeit aktiviert Submit – abgeschickt wird erst nach einem bewussten Klick.</p></section><div class="dna-actions"><button class="dna-btn" id="dnaBackSetup" type="button">${s.quickLobby?'← QUICK GAMES':'← SETUP'}</button><button class="dna-btn primary dna-blocking" id="dnaReady" type="button">ICH BIN BEREIT</button></div>`);$('#dnaBackSetup').addEventListener('click',()=>s.quickLobby?location.assign('../quick-games/'):setup());$('#dnaReady').addEventListener('click',()=>{const btn=$('#dnaReady');if(s.quickLobby){btn.disabled=true;btn.textContent='BEREIT WIRD GESPEICHERT …';startSharedQuickGame(true);return}const own=$(`[data-ready="${ownActual}"]`);own.textContent='BEREIT';own.classList.add('ready');btn.textContent='SPIEL STARTEN';btn.onclick=startGame})}
function ready(){renderReady();if(s.quickLobby)startSharedQuickGame(false)}
async function startGame(){if(s.quickLobby)return startSharedQuickGame();clearTimers();setChrome('LÄDT','CONTENT WIRD GEZOGEN',15);page(`<section class="dna-auth"><div class="dna-auth-card"><span class="dna-kicker">SERVERAUTORITATIV</span><h1>DNA WIRD VORBEREITET.</h1><p>Nur der erste freigegebene Hinweis wird an den Browser übertragen.</p></div></section>`);try{const res=await api('start',{categories:[...s.selected],termCount:s.termCount,pool:s.pool});s.token=res.state;s.previousHints=[];enterContent(res.content)}catch(err){if(String(err.message)==='LOGIN_REQUIRED')return authGate();showFatal(err)}}
...
function quickCommand(kind,value){quickSync?.request({kind,value,phase:quickSnapshot?.phaseKey})}
function startSharedQuickGame(markReady=true){
 if(markReady){const btn=$('#dnaReady');if(btn){btn.disabled=true;btn.textContent='WARTET AUF ALLE PLAYER …'}}
 if(quickSync){if(markReady)quickSync.request({kind:'ready'});return}
 clearTimers();setChrome('VERBINDET','',12);
 quickSync=new DnaQuickSync({send:sendQuickCommand,apply:applySharedQuickState,error:err=>{
  if(err){headerState.textContent=String(err.message)==='LOGIN_REQUIRED'?'ANMELDUNG ABGELAUFEN':'VERBINDUNG WIRD WIEDERHERGESTELLT';}
 }});
...
function sharedVoteMarkup(team){
 const rows=new Map(),slots=currentVoterSlots(),byId=new Map(slots.map(x=>[String(x.id),x]));
 for(const row of team.ideas){
   if(!row.idea)continue;
   const key=normalize(row.idea),item=rows.get(key)||{value:key,who:[]};
   const slot=byId.get(String(row.id||''))||slots.find(x=>x.is_me===Boolean(row.is_me));
   if(slot&&!item.who.some(x=>x.id===slot.id))item.who.push(slot);
   rows.set(key,item);
 }
...
function applySharedQuickState(snapshot){
 ...
 s.userVote=team.votes.find(r=>r.is_me)?.vote||null;
 s.teammateVote=team.votes.find(r=>!r.is_me)?.vote||team.botVote||null;
 headerState.textContent=({READY:'READY',IDEA:'IDEE',VOTE:'ABSTIMMUNG',FEEDBACK:'ERGEBNIS',REVEAL:'REVEAL',COUNTDOWN:'NÄCHSTER BEGRIFF',COMPLETE:'ERGEBNIS'})[stage];
 if(stage==='READY'){
  const btn=$('#dnaReady');if(btn&&team.players.find(p=>p.is_me)?.ready){btn.disabled=true;btn.textContent=`${team.players.filter(p=>p.ready).length} / ${team.players.length} PLAYER BEREIT`}
  for(const key of TEAM_ORDER){const row=$(`[data-ready="${key}"]`),ready=team.players.filter(p=>p.team===key).every(p=>p.ready);if(row){row.textContent=ready?'BEREIT':'WARTET';row.classList.toggle('ready',ready)}}
  return;
 }
 if(stage==='COMPLETE'){
 ...
 if(stage==='IDEA'){
  if(changed){
   ...
   const input=$('#dnaIdea'),save=$('#dnaIdeaSave'),skip=$('#dnaIdeaSkip');
   const send=value=>{if(quickIdeaPending)return;quickIdeaPending=true;save.disabled=true;skip.disabled=true;save.textContent='SPEICHERT …';quickCommand('idea',value)};
   input.addEventListener('input',()=>{save.disabled=quickIdeaPending||!input.value.trim()});
   input.addEventListener('keydown',e=>{if(e.key==='Enter'&&input.value.trim()){e.preventDefault();send(input.value.trim())}});
   save.addEventListener('click',()=>send(input.value.trim()));skip.addEventListener('click',()=>send(''));
   if(!team.solved&&!s.ideaDone)later(()=>{if(input.isConnected){input.focus({preventScroll:true});syncViewport()}},80);
  }
  if((s.ideaDone||team.solved)&&quickUiState!=='locked'){
   quickUiState='locked';const mine=team.ideas.find(r=>r.is_me);
   renderIdeaLocked(team.solved?'BEGRIFF GELÖST':'IDEE GESPEICHERT',team.solved?'WARTET AUF DIE ANDEREN TEAMS':mine?.idea||'KEINE IDEE');
  }
 }else if(stage==='VOTE'){
  if(changed){
   freezeCurrentHintStage();document.activeElement?.blur?.();syncViewport();
   gameShell(`<div class="dna-interaction-head"><strong>TEAM-ANTWORT</strong><span>ANTIPPEN ≠ ABSENDEN</span></div><div class="dna-vote-list" id="dnaVoteList"></div><div class="dna-consensus" id="dnaConsensus">NOCH KEINE EINIGKEIT</div><div class="dna-vote-actions"><button class="dna-btn" id="dnaVoteClear" type="button">AUSWAHL LÖSEN</button><button class="dna-btn primary" id="dnaSubmit" type="button" disabled>ANTWORT ABSENDEN</button></div>`,'ABSTIMMUNG');
   $('#dnaVoteList').addEventListener('click',e=>{const button=e.target.closest('[data-vote]');if(button)quickCommand('vote',button.dataset.vote)});
   $('#dnaVoteClear').addEventListener('click',()=>quickCommand('vote',''));
   $('#dnaSubmit').addEventListener('click',()=>quickCommand('submit'));
  }
  if(team.solved||team.submission){
   const key=team.solved?'solved':'submitted';if(quickUiState!==key){quickUiState=key;team.solved?renderIdeaLocked('BEGRIFF GELÖST','WARTET AUF DIE ANDEREN TEAMS'):renderSubmitted(team.submission.answer)}
  }else{
   const markup=sharedVoteMarkup(team);if(quickUiState!==markup){quickUiState=markup;$('#dnaVoteList').innerHTML=markup}
   renderVotes();
  }
 }else if(stage==='FEEDBACK'&&changed){
  freezeCurrentHintStage();document.activeElement?.blur?.();
...
function startIdea(){clearTimers();s.screen='GAME';s.idea='';s.ideaDone=Boolean(s.current.redSolved);s.teammateIdea=null;s.teammateReady=Boolean(s.current.redSolved);s.opponentsReady=false;s.userVote=null;s.teammateVote=null;s.submitted=false;s.outcome=null;s.answerProof=null;const solved=s.current.redSolved;gameShell(solved?`<div class="dna-lock-state"><div><strong>BEGRIFF GELÖST</strong><span>DEIN TEAM SPIELT DIESEN BEGRIFF NICHT WEITER.</span></div></div>`:`<div class="dna-interaction-head"><strong>DEINE IDEE</strong><span>GEHEIM · NUR DU</span></div><div class="dna-idea-form"><input class="dna-input" id="dnaIdea" type="text" autocomplete="off" autocorrect="off" spellcheck="false" autocapitalize="characters" enterkeyhint="done" maxlength="80" placeholder="Antwort eingeben …"><button class="dna-btn primary dna-save" id="dnaIdeaSave" type="button" disabled>IDEE SPEICHERN</button></div><button class="dna-btn dna-skip" id="dnaIdeaSkip" type="button">KEINE IDEE</button>`,'IDEE');
 if(!solved){const input=$('#dnaIdea'),save=$('#dnaIdeaSave');input.addEventListener('input',()=>save.disabled=!input.value.trim());input.addEventListener('keydown',e=>{if(e.key==='Enter'&&input.value.trim()){e.preventDefault();save.click()}});save.addEventListener('click',()=>saveQuickIdea(input.value.trim()));$('#dnaIdeaSkip').addEventListener('click',()=>saveQuickIdea(''));later(()=>{try{input.focus({preventScroll:true})}catch(_){input.focus()}syncViewport()},80);if(s.quickHumanTeammate)pollQuickIdea();else later(()=>{s.teammateIdea=teammateIdea();s.teammateReady=true;maybeFinishIdea()},900+Math.random()*800)}
...
async function saveQuickIdea(value){
 if(s.ideaDone)return;s.idea=value;s.ideaDone=true;renderIdeaLocked(value?'IDEE GESPEICHERT':'ÜBERSPRUNGEN',value||'KEINE IDEE');
 if(!s.quickHumanTeammate)return maybeFinishIdea();
 try{const team=await quickRpc('submit_quick_dna_idea',{p_lobby_id:s.quickLobby.lobby_id,p_term_no:s.current.termNo,p_hint_no:s.current.hintNo,p_idea:value});applyQuickIdeaState(team)}catch(err){showFatal(err)}
}
function applyQuickIdeaState(team){const other=(team?.ideas||[]).find(row=>!row.is_me);if(other?.submitted){s.teammateIdea=other.idea||null;s.teammateReady=true}maybeFinishIdea()}
async function pollQuickIdea(){if(!s.quickHumanTeammate||s.screen!=='GAME'||s.teammateReady)return;try{const team=await quickRpc('get_quick_dna_team_state',{p_lobby_id:s.quickLobby.lobby_id,p_term_no:s.current.termNo,p_hint_no:s.current.hintNo});applyQuickIdeaState(team);if(!s.teammateReady)later(pollQuickIdea,500)}catch(err){showFatal(err)}}
...
function startVote(){clearTimers();freezeCurrentHintStage();try{document.activeElement?.blur?.()}catch(_){}syncViewport();teammateVoteSeq=0;clearTimeout(reconsiderTimer);const solved=s.current.redSolved;if(solved){s.teammateReady=true;s.opponentsReady=false;gameShell(`<div class="dna-lock-state"><div><strong>TEAMABSTIMMUNG LÄUFT</strong><span>DEIN TEAM HAT BEREITS GELÖST.</span></div></div>`,'ABSTIMMUNG');later(()=>{s.opponentsReady=true;clearTimers();resolveVote(true)},900+Math.random()*600);startTimer(VOTE_PHASE_MS,()=>resolveVote(true));return}
 ...
 content.querySelectorAll('[data-vote]').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  s.teammateVote=null;
  renderVotes();
  if(s.quickHumanTeammate)shareQuickVote(s.userVote);else reconsiderVoteNow(seq,s.userVote);
 }));$('#dnaVoteClear').addEventListener('click',()=>{s.userVote=null;s.answerProof=null;if(s.quickHumanTeammate)shareQuickVote(null);renderVotes()});$('#dnaSubmit').addEventListener('click',submitVote);
 if(s.quickHumanTeammate)pollQuickVote();
 later(()=>{
   // Never overwrite a user-triggered reconsideration with the stale initial bot vote.
   if(s.quickHumanTeammate||s.userVote||s.submitted||teammateVoteSeq>0)return;
...
async function shareQuickVote(value){try{const team=await quickRpc('submit_quick_dna_vote',{p_lobby_id:s.quickLobby.lobby_id,p_term_no:s.current.termNo,p_hint_no:s.current.hintNo,p_vote:value||''});applyQuickVoteState(team)}catch(err){showFatal(err)}}
function applyQuickVoteState(team){const mine=(team?.votes||[]).find(row=>row.is_me),other=(team?.votes||[]).find(row=>!row.is_me);if(mine)s.userVote=mine.vote||null;if(other)s.teammateVote=other.vote||null;renderVotes();if(team?.submission&&!s.submitted)submitSharedAnswer(team.submission.answer)}
async function pollQuickVote(){if(!s.quickHumanTeammate||s.screen!=='GAME'||s.submitted)return;try{const team=await quickRpc('get_quick_dna_team_state',{p_lobby_id:s.quickLobby.lobby_id,p_term_no:s.current.termNo,p_hint_no:s.current.hintNo});applyQuickVoteState(team);if(!s.submitted)later(pollQuickVote,500)}catch(err){showFatal(err)}}
function voteCard(value,source,key){const display=value==='__NO__'?'KEINE ANTWORT':value,voters=currentVoterSlots().map(v=>`<i data-voter-id="${esc(v.id)}">${esc(v.marker)}</i>`).join('');return `<button class="dna-vote" type="button" data-vote="${esc(value)}" data-key="${key}"><span><strong>${esc(display)}</strong><small>${esc(source)}</small></span><span class="dna-voters">${voters}</span></button>`}
...
function renderVotes(){const slots=currentVoterSlots(),mine=slots.find(x=>x.is_me)||slots[0],mate=slots.find(x=>!x.is_me)||slots[1];content.querySelectorAll('[data-vote]').forEach(btn=>{const v=btn.dataset.vote;btn.classList.toggle('active',s.userVote===v);btn.querySelectorAll('[data-voter-id]').forEach(icon=>{const id=String(icon.dataset.voterId||''),on=id===String(mine?.id||'')?s.userVote===v:id===String(mate?.id||'')?s.teammateVote===v:false;icon.classList.toggle('on',on)})});const consensus=Boolean(s.userVote&&s.teammateVote&&s.userVote===s.teammateVote);const line=$('#dnaConsensus'),submit=$('#dnaSubmit');if(line){line.textContent=consensus?'EINIGKEIT · SUBMIT IST FREIGEGEBEN':'NOCH KEINE EINIGKEIT';line.classList.toggle('dna-submit-ok',consensus)}if(submit)submit.disabled=!consensus||s.submitted}
async function submitVote(){if(s.submitted||!s.userVote||s.userVote!==s.teammateVote)return;if(s.quickHumanTeammate){try{const team=await quickRpc('submit_quick_dna_team_answer',{p_lobby_id:s.quickLobby.lobby_id,p_term_no:s.current.termNo,p_hint_no:s.current.hintNo});return submitSharedAnswer(team?.submission?.answer)}catch(err){return showFatal(err)}}return submitSharedAnswer(s.userVote)}
async function submitSharedAnswer(answer){if(s.submitted||!answer)return;s.submitted=true;renderVotes();const noAnswer=answer==='__NO__';try{const res=await api('submit',{state:s.token,termRef:s.current?.termRef||'',answerProof:s.answerProof||'',answer:noAnswer?'':answer,noAnswer});s.token=res.state;s.current=res.content;s.outcome=res.outcome;renderSubmitted(answer);if(s.opponentsReady)resolveVote(false)}catch(err){showFatal(err)}}
...
async function showQuickRanking(scores){
 ...
 try{
  ...
  for(let attempt=0;Number(lobby.result_count||0)<(lobby.players||[]).length&&attempt<120;attempt++){
   ...
   const shared={};TEAM_ORDER.forEach((key,index)=>{const teamPlayers=(lobby.players||[]).filter(p=>Math.floor((Number(p.seat)-1)/2)===index),humanScores=teamPlayers.map(p=>p.score).filter(v=>v!==null&&v!==undefined).map(Number);shared[key]=humanScores.length?Math.max(...humanScores):Number(lobby.bot_scores?.[key]||0)});
  showRanking(shared);
...
function resetAll(){clearTimers();resolving=false;clearTimeout(reconsiderTimer);s={screen:'SETUP',selected:new Set(['countries']),termCount:5,pool:'BALANCED',token:null,current:null,previousHints:[],idea:'',ideaDone:false,teammateIdea:null,teammateReady:false,opponentsReady:false,userVote:null,teammateVote:null,submitted:false,outcome:null,gameScores:null,ranking:null,mergeComplete:false,quickLobby:null,quickTeamMap:null,quickHumanTeammate:false};setup()}
async function boot(){const session=await authSession();if(!session)return authGate();themeCatalog=await loadThemeCatalog();renderThemeQa();try{if(QUICK_LOBBY_ID)await loadQuickLobby();if(!s.quickLobby)return setup();if(s.quickLobby.setup_status==='READY'){applyQuickSetup();ready()}else if(s.quickLobby.is_host)setup();else waitForQuickSetup()}catch(err){showFatal(err)}}
boot();
})();
