...
function ensureMoreLessAssets(){
  ...
  moreLessAssetsPromise=new Promise((resolve,reject)=>{
    if(!document.querySelector('link[data-more-less-css]')){
      const link=document.createElement('link');
      link.rel='stylesheet';link.href=`assets/css/more-or-less-game.css?v=${VERSION}&fix=molux7`;link.dataset.moreLessCss='1';
      document.head.appendChild(link);
    }
    ...
    const script=document.createElement('script');
    script.src=`assets/js/11-more-or-less-game.js?v=${VERSION}&fix=molux7`;
