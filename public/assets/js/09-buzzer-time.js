      ...
      <section class="bzt-standard-card bzt-merge-card is-game" data-bzt-merge-card>
        ...
        <div class="bzt-standard-rows bzt-merge-rows">
          ${rows.map((r,i)=>`<div class="bzt-standard-row bzt-merge-row" data-bzt-merge-row data-old-rank="${Number(r.old_rank||i+1)}" data-new-rank="${Number(r.new_rank||i+1)}">
            ...
            <strong class="bzt-merge-points"><span class="bzt-award-value">+${Number(r.added_points||0)}</span><span class="bzt-points-equation"><b class="bzt-base-points">${Number(r.old_points||0)}</b><em>+</em><strong class="bzt-award-points">${Number(r.added_points||0)}</strong></span><span class="bzt-total-points">${Number(r.new_points||0)}</span></strong>
...
function animateBuzzerMerge(){
  ...
  postgameLater(()=>{
    ...
    rows.forEach(r=>{
      ...
      if(mv){const delta=oldRank-newRank;mv.textContent=buzzerMovement(delta);mv.className='bzt-rank-move '+(delta>0?'up':delta<0?'down':'same')+' visible'}
    });
  },5340);
  postgameLater(()=>{
    if(run!==postgameRun)return;
