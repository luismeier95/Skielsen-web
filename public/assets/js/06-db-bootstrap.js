(async()=>{
  const APP_VERSION=window.SKIELSEN_VERSION||'15.1.140';
  ...
  try{
    ...
    const resumeEntry=window.skielsenHistory?.current?.()||null;
    if(existing){
      if(resumeEntry?.area==='tournament'&&resumeEntry?.data?.tournamentId){
        const tid=resumeEntry.data.tournamentId;
        let stored=null;
        try{const raw=localStorage.getItem('skielsen.db.lastTournament');const parsed=raw?JSON.parse(raw):null;if(parsed?.tournament_id===tid)stored=parsed}catch(_){}
        lastResult={...(stored||{}),tournament_id:tid,status:'LIVE',user_id:existing?.user?.id||stored?.user_id||null};
        await showStartedStep(lastResult,'none');
        await window.skielsenHistory?.restore?.(resumeEntry);
      }else if(resumeEntry?.area==='workflow'){
        const tid=resumeEntry?.data?.tournamentId||null;
        if(tid){
          let stored=null;
          try{const raw=localStorage.getItem('skielsen.db.lastTournament');const parsed=raw?JSON.parse(raw):null;if(parsed?.tournament_id===tid)stored=parsed}catch(_){}
          lastResult={...(stored||{}),tournament_id:tid,status:stored?.status||'LOBBY',user_id:existing?.user?.id||stored?.user_id||null};
        }
        const restored=await window.skielsenHistory?.restore?.(resumeEntry);
        if(!restored)await enterAccountHome(existing,'replace');
      }else{
  ...
  document.getElementById('dbbHomeQuickGames').addEventListener('click',()=>{window.location.href='quick-games/'});
