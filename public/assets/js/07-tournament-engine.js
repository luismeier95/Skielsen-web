(()=>{
'use strict';
const VERSION=window.SKIELSEN_VERSION||'15.1.133';
...
function isTeamTournament(){return String(runtime?.mode||'').toUpperCase()==='TEAM'}
function voteFeatureEnabled(type){
 if(!isTeamTournament())return false;
 const t=String(type||'').toUpperCase();
 if(t==='MVP')return feature('feature.mvp_voting');
 if(t==='LVP')return feature('feature.lvp_voting');
 return false
}
function mvpFeatureEnabled(){return voteFeatureEnabled('MVP')}
function activeVoteTypes(){return ['MVP','LVP'].filter(voteFeatureEnabled)}
function voteFeaturesEnabled(){return activeVoteTypes().length>0}
...
function syncBettingFeatureVisibility(){
 ...
 const adminWallet=q('#adminWallet');if(adminWallet)adminWallet.hidden=!enabled;
...
function syncVoteFeatureVisibility(){
 const enabled=voteFeaturesEnabled(),pop=q('#mvpCompletePopup');
 if(pop&&!enabled){pop.hidden=true;pop.classList.remove('is-visible')}
 const page=q('#mvpVotePage');if(page)page.dataset.featureEnabled=enabled?'true':'false';
 if(!enabled)selectedMvpCandidate=null;
}
...
function syncJokerFeatureVisibility(){
 ...
 if(!enabled){
   ...
   const pick=q('#v1536PickOpponentOverlay');if(pick)pick.hidden=true;
 }
}
...
function serverVoteApplies(g=gameNow(),type=g?.vote?.type||null){return !!(client&&runtime?.tournament_id&&g?.tournament_game_id&&voteFeatureEnabled(type))}
...
function featureContextAllowsText(type,text=''){
 const t=String(type||'').toUpperCase(),x=String(text||'').toUpperCase();
 if(!feature('feature.betting')&&(t==='UPSET'||/BETTING|WETTE|QUOTE|SIEGCHANCE|COINS?|WALLET|AUSZAHLUNG/.test(x)))return false;
 if(!feature('feature.joker')&&(t.includes('JOKER')||x.includes('JOKER')))return false;
 if(!voteFeatureEnabled('MVP')&&(t==='MVP'||/\bMVP\b/.test(x)))return false;
 if(!voteFeatureEnabled('LVP')&&(t==='LVP'||/\bLVP\b|\bDORSCH\b/.test(x)))return false;
 return true;
}
function contextualNews(){return (state?.news||[]).filter(n=>featureContextAllowsText(n.type,(n.headline||'')+' '+(n.body||'')))}
function contextualAudit(){return (state?.audit||[]).filter(x=>featureContextAllowsText('AUDIT',x.text||''))}
...
function buildParticipants(rt){
 ...
 if(String(rt.mode).toUpperCase()==='TEAM'){
   ...
   rows.forEach(pr=>{
     ...
     byColor.set(pr.identity_color,{id:pr.participant_id,name:a?.name||('PLAYER '+COLOR_DE[pr.identity_color]),color:pr.identity_color,actorIds:a?[a.id]:[],seed:pr.seed??null,soloMemberId:pr.solo_member_id||null,isBotParticipant:!!a?.isBot,testBotId:a?.isBot?a.id:null});
   });
...
async function prepareCurrentGame(){const g=gameNow();if(!g)return false;if(g.phase==='PLANNED')return await preparePlannedGame(g);if(g.phase==='PREPARING'&&feature('feature.joker')&&g.joker?.awaitingPick){startJokerResolutionPolling(g);return false}if(!g.matches?.length)g.matches=createScheduledMatches(g,state.currentGameIndex,state.participants);if(g.phase!=='ACTIVE'){g.phase='ACTIVE';openCurrentMarket(g)}return true}
...
function settleBets(m,winnerId){if(!feature('feature.betting')||!m)return;for(const b of state.bets[m.id]||[]){if(b.status!=='PLACED')continue;if(serverBettingApplies()&&!actor(b.actorId)?.isBot)continue;if(b.participantId===winnerId){b.status='WON';b.payout=Math.ceil(Number(b.stake||0)*Number(b.acceptedOdds||0));state.wallets[b.actorId]=(state.wallets[b.actorId]||0)+b.payout;state.transactions.push({id:'tx-pay-'+b.id,type:'PAYOUT',actorId:b.actorId,amount:b.payout,betId:b.id,at:nowLabel(),reason:m.id})}else{b.status='LOST';b.payout=0}}}
...
function showResultPopup(m,g,winnerId,onContinue=null){
 ...
 pop.classList.remove('is-win','is-loss','is-shared','is-visible');pop.classList.toggle('is-shared',shared);if(bettingEnabled&&won)pop.classList.add('is-win');else if(bettingEnabled&&lost)pop.classList.add('is-loss');
...
function beginCanonicalMergedPostGameFlow(g,m,winnerId){
 if(!g)return false;
 window.skielsenInApp?.finishAndExit?.();
 if(g.joker){g.joker.revealed=true;g.joker.revealAcknowledged=true;g.joker.resolved=true;g.joker.locked=true}
 const proceed=()=>{startPostGameVote(g);renderAll();saveSoon();setTimeout(routeToCurrentWork,0)};
 if(feature('feature.betting')&&serverBettingApplies()&&m&&winnerId){
   void client.rpc('settle_betting_market',{p_tournament_game_id:g.tournament_game_id,p_match_sequence_no:currentMatchSequence(g,m),p_winner_participant_id:winnerId}).then(({data,error})=>{if(!error&&data)applyServerBettingState(data,g,m);else if(error)console.warn('Canonical merged postgame betting settlement failed',error);proceed()});return true
 }
 proceed();return true
}
function routeToCurrentWork(){
 ...
 if(g.vote&&!g.vote.finalized&&requiredPostGameVoteTypes().includes(g.vote.type)){if(!g.vote.votes?.[state.userActorId])show('mvpVote');else show('home');return}
 if(g.vote&&!requiredPostGameVoteTypes().includes(g.vote.type)){g.vote=null;if(['VOTING_MVP','VOTING_LVP','AWARD_REVEAL'].includes(g.phase))g.phase='RESULTS'}
...
function revealConfigFromPayload(payload,g){
 ...
 return {
   ...
   ownerTeamColor:(owner.color_key?(({BLUE:'var(--core-blue)',RED:'var(--core-red)',YELLOW:'var(--core-yellow)',GREEN:'var(--core-green)'})[String(owner.color_key).toUpperCase()]||owner.color):owner.color)||jokerTeamColorHex(participant(g?.joker?.accepted?.participantId)),
...
async function refreshServerVoteState(g=gameNow(),type=g?.vote?.type||null){
 if(!type||!serverVoteApplies(g,type))return null;
 try{
...
function startVotePolling(g=gameNow(),type=g?.vote?.type||null){
 if(votePollTimer||!type||!serverVoteApplies(g,type))return;
...
function requiredPostGameVoteTypes(){const out=[];if(voteFeatureEnabled('MVP'))out.push('MVP');if(voteFeatureEnabled('LVP'))out.push('LVP');return out}
...
function startPostGameVoteType(g,type){
 if(!g||!type||!requiredPostGameVoteTypes().includes(String(type).toUpperCase()))return false;
...
async function submitVote(voterId,candidateId){
 ...
 if(voterId===state.userActorId&&serverVoteApplies(g,g.vote.type)){
...
function showVoteResult(g,type){
 const pop=q('#mvpCompletePopup'),r=g?.postGameVoteResults?.[type];if(!voteFeatureEnabled(type)||!pop||!r){if(pop)pop.hidden=true;return false}
...
function finishTournament(){
 if(state.finalBonusApplied||state.tournamentDone)return;
 state.finalBonusApplied=true;
 if(mvpFeatureEnabled()){
   const bonus=Number(runtime?.scoring?.config?.mvpTeamBonus??runtime?.scoring?.config?.mvp_team_bonus??10);
   const vals=state.participants.map(p=>({p,count:Number(state.rankings[p.id]?.mvp||0)}));
   const max=Math.max(...vals.map(x=>x.count),0),leaders=vals.filter(x=>x.count===max&&max>0);
   if(leaders.length===1&&bonus!==0){
     state.rankings[leaders[0].p.id].points+=bonus;
     state.rankings[leaders[0].p.id].bonus+=bonus;
     addAudit('MVP TEAM BONUS '+(bonus>=0?'+':'')+bonus+' · '+leaders[0].p.name);
     addNews('MVP-TEAM-BONUS ENTSCHEIDET.',leaders[0].p.name+' erhält '+(bonus>=0?'+':'')+bonus+' Punkte für die meisten MVP-Titel.','TOURNAMENT')
   }else if(leaders.length>1){
     addAudit('MVP TEAM BONUS · GLEICHSTAND · KEIN BONUS');
     addNews('MVP-TITEL IM GLEICHSTAND.','Bei Gleichstand wird kein MVP-Team-Bonus vergeben.','SYSTEM')
   }
 }
 state.tournamentDone=true;
 addNews('TURNIER ABGESCHLOSSEN.','Alle ausgewählten Games wurden abgeschlossen.','TOURNAMENT');
 addAudit('TOURNAMENT COMPLETE')
}
...
function openJokerResolutionNotice(ss){
 ...
 if(!feature('feature.joker')){if(d)d.hidden=true;return false}if(!d||!card||!g)return false;
...
function openPendingPickDialog(pp){
 ...
 if(!feature('feature.joker')){if(overlay)overlay.hidden=true;return false}if(!g||!p||!overlay||!card||!choices||!grid||!confirm)return false;if(!g.joker?.awaitingPick)return false;if(overlay.hidden===false&&pendingPickDialogGameId===g.tournament_game_id)return;
...
async function preparePlannedGame(g){
 if(!g||g!==gameNow()||g.phase!=='PLANNED')return false;
 if(!feature('feature.joker')){
   g.phase='PREPARING';
   g.joker=g.joker||{submissions:[],accepted:null,resolved:true,locked:true,awaitingPick:false};
   g.joker.submissions=[];g.joker.accepted=null;g.joker.awaitingPick=false;g.joker.resolved=true;g.joker.locked=true;
   renderAll();saveSoon();
   if(client&&g.tournament_game_id&&isAdmin()){
     try{
       const {error}=await client.rpc('prepare_tournament_game',{p_tournament_game_id:g.tournament_game_id});
       if(error)throw error
     }catch(e){
       console.warn('Server prepare transition returned an error',e);
       g.phase='PLANNED';renderAll();saveSoon();
       showFlowToast('MATCH','VORBEREITUNG FEHLGESCHLAGEN',String(e?.message||e),2600);
       return false
     }
   }
   finishJokerPreparation(g);
 ...
 const knownJokerCount=knownPendingJokerCount(g);
 if(knownJokerCount>1)openMultiJokerDialog(g,knownJokerCount);
 g.phase='PREPARING';
 g.joker=g.joker||{submissions:[],accepted:null,resolved:false,locked:false,awaitingPick:false};
 g.joker.locked=true;renderAll();saveSoon();
 if(client&&g.tournament_game_id&&isAdmin()){
   try{
     const {data,error}=await client.rpc('prepare_tournament_game',{p_tournament_game_id:g.tournament_game_id});
     if(error)throw error;
     applyPreparedJokerResult(g,data||{});
     await refreshServerJokerBoard();
     return true
   }catch(e){
     console.warn('Server prepare transition returned an error',e);
     try{
       const {data}=await client.rpc('get_tournament_game_joker_state',{p_tournament_game_id:g.tournament_game_id});
       if(data&&data.status==='PREPARING'){applyPreparedJokerResult(g,data);await refreshServerJokerBoard();return true}
     }catch(_){}
   }
 }
 return fallbackResolvePlannedJokers(g)
}
...
function matchPairingProvisional(g){return feature('feature.joker')&&(g?.phase==='PLANNED'||(g?.phase==='PREPARING'&&g?.joker?.awaitingPick))}
function matchStatusMarkup(m,g){const provisional=matchPairingProvisional(g);const txt=provisional?(g?.joker?.awaitingPick?'WARTET AUF JOKER':'VORLÄUFIG'):statusLabel(m?.status);const cls=m?.status==='LIVE'?'is-live':m?.status==='BETTING_OPEN'?'is-betting':'';return `<span class="v20-match-status ${cls}">${esc(txt)}</span>`}
function compactMatchInner(m,g,gi){
 ...
 const provisional=matchPairingProvisional(g),a=!provisional&&m?.a?participant(m.a):null,b=!provisional&&m?.b?participant(m.b):null;
...
function largePairingMarkup(m,g){
 ...
 const provisional=matchPairingProvisional(g);
...
function renderHome(){
 ...
 if(hero){setText('h1',runtime.tournament_name||window.SKIELSEN_DB_CONTEXT?.name||runtime.name||'SKIELSEN CUP',hero);setText('p',state.tournamentDone?'TURNIER ABGESCHLOSSEN':g?`GAME ${String(state.currentGameIndex+1).padStart(2,'0')} / ${String(state.games.length).padStart(2,'0')} · ${g.name}`:'KEIN GAME');const pe=q('.home-player-entry strong',hero);if(pe)pe.textContent=(ua?.name||'PLAYER').toUpperCase()+' · '+(up?.name||'');const av=q('.home-player-avatar',hero);if(av)av.textContent=initials(ua?.name)}
 ...
 if(mvp){const need=!!(g?.vote&&requiredPostGameVoteTypes().includes(g.vote.type)&&!g.vote.finalized&&!g.vote.votes?.[state.userActorId]);mvp.hidden=!need;mvp.classList.toggle('mandatory-open',need);mvp.onclick=()=>show('mvpVote');q('strong',mvp).textContent=g?.vote?.type==='LVP'?`${g.name} LVP WÄHLEN`:`${g?.name||''} MVP WÄHLEN`;q('p',mvp).textContent=g?.vote?.type==='LVP'?'Stimme für einen anderen Player. Dein eigener Teampartner ist erlaubt.':'Stimme für einen Spieler außerhalb deines Teams.'}
 ...
 const story=q('#homePage .home-story');if(story){const n=contextualNews()[0];q('small',story).textContent=n?`${n.type} · ${n.at}`:'SKIELSENSPORT';q('strong',story).textContent=n?.headline||'Noch kein Ergebnis im Turnier.'}
}
...
function concludeSharedPlacements(placements,source='RESULT',values=null){
 ...
 const winSel=feature('feature.betting')?selectionSnapshot(state.oddsSnapshots[m.id],winnerId):null;if(winSel?.probability<.4)addNews('MULTI-PARTICIPANT UNDERDOG GEWINNT.',`${teamName(participant(winnerId))} gewinnt ${g.name} mit ${Math.round(winSel.probability*100)} % eingefrorener Siegchance.`,'UPSET');else addNews(`${teamName(participant(winnerId))} GEWINNT ${g.name.toUpperCase()}.`,placements.map((pid,i)=>`${i+1}. ${teamName(participant(pid))}`).join(' · '),'MATCH_REPORT');
...
function v1536ShowBetSaved(bet){const modal=q('#v1536BetSavedModal');if(!feature('feature.betting')){if(modal)modal.hidden=true;return false}if(!modal||!bet)return false;const p=participant(bet.participantId);setText('#v1536BetTipTeam',teamName(p));const bar=q('#v1536BetTipBar');if(bar){bar.className=marker(p?.color);bar.style.background=({BLUE:'var(--core-blue)',RED:'var(--core-red)',YELLOW:'var(--core-yellow)',GREEN:'var(--core-green)'}[p?.color]||'var(--theme-muted)')}setText('#v1536BetSavedStake',fmt(Math.round(Number(bet.stake||0))));setText('#v1536BetSavedPayout',fmt(Math.ceil(Number(bet.stake||0)*Number(bet.acceptedOdds||0))));modal.hidden=false}
...
function renderMatchDetailControlEntry(m,g){
 ...
 if(g.phase==='PLANNED'){
   if(feature('feature.joker')){title.innerHTML='<span class="v1536-loader">JOKERRUNDE...</span>';block.classList.add('is-waiting');btn.textContent=isAdmin()?('JOKER SCHLIESSEN → '+(feature('feature.betting')?'BETTING':'MATCH')):'WARTET AUF ADMIN'}
   else if(feature('feature.betting')){title.textContent='BETTING VORBEREITEN';btn.textContent=isAdmin()?'BETTING ÖFFNEN':'WARTET AUF ADMIN'}
   else{title.textContent='MATCH BEREIT';btn.textContent=isAdmin()?'MATCH STARTEN':'WARTET AUF ADMIN'}
   btn.disabled=!isAdmin();btn.classList.toggle('waiting',!isAdmin())
 }else if(g.phase==='PREPARING'){
   const waits=feature('feature.joker')&&!!g.joker?.awaitingPick;title.innerHTML=waits?'<span class="v1536-loader">JOKER-AKTION...</span>':'<span class="v1536-loader">VORBEREITUNG...</span>';block.classList.add('is-waiting');const own=waits&&g.joker?.accepted?.participantId===state.userParticipantId&&serverJokerBoard?.pending_pick;if(own){btn.textContent='GEGNER WÄHLEN';btn.disabled=false}else{btn.textContent=waits?'WARTET AUF GEGNERWAHL':'WIRD VORBEREITET';btn.disabled=true;btn.classList.add('waiting')}
...
function renderRanking(){
 ...
 if(!page)return;
 const showMvp=mvpFeatureEnabled(),rows=rankingsSorted(),table=q('.team-ranking-table',page);
 if(table){
  const header='<div class="ranking-row ranking-labels '+(showMvp?'':'without-mvp')+'"><span>#</span><span>TEILNEHMER</span><span>PUNKTE</span><span class="medal gold">1ST</span><span class="medal silver">2ND</span><span class="medal bronze">3RD</span><span>LAST</span>'+(showMvp?'<span>MVP</span>':'')+'</div>';
  table.innerHTML=header+rows.map((x,i)=>'<div class="ranking-row team-row '+(showMvp?'':'without-mvp')+' '+(x.p.id===state.userParticipantId?'current-team':'')+'"><b class="rank-pos">'+String(i+1).padStart(2,'0')+'</b><div class="rank-team"><i class="'+marker(x.p.color)+'"></i><strong>'+esc(x.p.name)+'</strong></div><b class="rank-points">'+fmt(x.r.points)+'</b><span>'+x.r.first+'</span><span>'+x.r.second+'</span><span>'+x.r.third+'</span><span>'+x.r.last+'</span>'+(showMvp?'<span>'+x.r.mvp+'</span>':'')+'</div>').join('');
 }
 const leaders=q('#v20RankingLeaders',page);
 if(leaders){
  const defs=[['PUNKTE','points'],['FIRST PLACES','first'],['SECOND PLACES','second'],['THIRD PLACES','third']];
  if(showMvp)defs.push(['MVP TITEL','mvp']);
 ...
 const summary=q('.ranking-breakdown summary span',page);if(summary)summary.textContent=showMvp?'PLACEMENTS · MVP':'PLACEMENTS';
 const placements=runtime?.scoring?.placements||{'1':10,'2':5,'3':3,'4':0};
 const bonus=Number(runtime?.scoring?.config?.mvpTeamBonus??runtime?.scoring?.config?.mvp_team_bonus??10);
 const note=q('.ranking-rule-note',page);
 if(note)note.textContent='PUNKTE PRO GAME: '+Object.keys(placements).sort((a,b)=>Number(a)-Number(b)).map(k=>k+'. '+fmt(placements[k])).join(' · ')+(showMvp?' · MVP-TEAM-BONUS '+(bonus>=0?'+':'')+fmt(bonus)+' NACH TURNIERENDE':'');
}
function renderBets(){
...
function renderAdmin(){
 ...
 const adminWallet=q('#adminWallet');if(adminWallet)adminWallet.hidden=!feature('feature.betting');const coinSel=q('#coinAdjustPlayer');if(coinSel&&feature('feature.betting')){const cur=coinSel.value;coinSel.innerHTML=state.actors.map(a=>`<option value="${esc(a.id)}">${esc(a.name.toUpperCase())} · ${esc(actorParticipant(a)?.name||'')}${a.isBot?' · BOT':''}</option>`).join('');if(state.actors.some(a=>a.id===cur))coinSel.value=cur}
 const roster=q('#v20AdminRoster');if(roster)roster.innerHTML=state.actors.map(a=>{const p=actorParticipant(a);return `<div><b>${esc(a.name)}</b><span><i class="${marker(p?.color)}"></i>${esc(p?.name||'NICHT ZUGEORDNET')}</span><em>${a.isBot?'BOT':'PLAYER'}</em></div>`}).join('')||'<div class="v20-empty">KEINE PLAYER GELADEN.</div>';const ag=q('#adminGameList');if(ag)ag.innerHTML=state.games.map((x,i)=>`<div class="${i===state.currentGameIndex?'current':''}"><b>${String(i+1).padStart(2,'0')}</b><span>${esc(x.name)}</span><em>${esc(x.phase)}</em></div>`).join('')||'<div class="v20-empty">KEINE GAMES GELADEN.</div>';const ar=q('#auditLogRows');if(ar)ar.innerHTML=contextualAudit().slice(0,18).map(x=>`<div class="audit-row"><b>${esc(x.at)}</b><span>${esc(x.text)}</span></div>`).join('')||'<div class="v20-empty">NOCH KEINE AUDIT-EINTRÄGE.</div>'
}
...
function renderGameControl(){
 ...
 else if(g?.phase==='PLANNED'){if(title)title.textContent=feature('feature.joker')?'JOKERRUNDE OFFEN':feature('feature.betting')?'BETTING NOCH NICHT GEÖFFNET':'MATCH BEREIT';if(copy)copy.textContent=feature('feature.joker')?'Joker können bis zum Start der Spielvorbereitung gesetzt, geändert oder zurückgezogen werden.':feature('feature.betting')?'Öffne den Wettmarkt. Danach startet der Match, sobald alle erforderlichen Entscheidungen vorliegen.':'Der Match kann direkt gestartet werden.';setHoldLabel(start,isAdmin()?(feature('feature.joker')?'JOKER SCHLIESSEN':feature('feature.betting')?'BETTING ÖFFNEN':'MATCH STARTEN'):'WARTET AUF ADMIN',!isAdmin())}
 else if(g?.phase==='PREPARING'){const waits=feature('feature.joker')&&!!g?.joker?.awaitingPick;if(title)title.textContent=waits?'WARTET AUF JOKER-AKTION':'SPIEL WIRD VORBEREITET';if(copy)copy.textContent=waits?'Ein akzeptierter PICK OPPONENT Joker muss zuerst abgeschlossen werden.':feature('feature.joker')?'Joker werden aufgelöst und die endgültigen Paarungen vorbereitet.':'Paarungen und Match-Lifecycle werden vorbereitet.';setHoldLabel(start,waits?'WARTET AUF GEGNERWAHL':'SPIEL WIRD VORBEREITET',true);panel?.classList.add('is-waiting')}
 ...
 else {if(title)title.textContent='MATCH ABGESCHLOSSEN';if(copy)copy.textContent=feature('feature.betting')?'Ergebnis und Betting-Abrechnung sind gespeichert.':'Ergebnis ist gespeichert.';setHoldLabel(start,'ABGESCHLOSSEN',true)}
...
function renderProfile(){const a=actor(state.selectedProfileActorId)||userActor(),p=actorParticipant(a),page=q('#profilePage');if(!a||!page)return;const voteTypes=activeVoteTypes(),showAwards=voteTypes.length>0,showMvp=mvpFeatureEnabled();qa('[data-player-section="achievements"]').forEach(x=>x.hidden=!showAwards);const achievementsView=q('#achievementsView',page);if(achievementsView)achievementsView.hidden=!showAwards;if(!showAwards&&page.dataset.playerSectionOpen==='achievements'){page.dataset.playerSectionOpen='overview';qa('[data-player-section]').forEach(x=>x.classList.toggle('is-current',x.dataset.playerSection==='overview'))}setText('.player-name',a.name.toUpperCase(),page);setText('.avatar',initials(a.name),page);const rr=state.rankings[p?.id]||{},rank=rankingsSorted().findIndex(x=>x.p.id===p?.id)+1,history=state.matchHistory.filter(h=>historyIncludesParticipant(h,p?.id));const wins=history.filter(h=>historyOutcome(h,p?.id)?.win).length;setText('.player-meta',showMvp?`${history.length} MATCHES · ${wins} WINS · ${rr.mvp||0} MVP`:`${history.length} MATCHES · ${wins} WINS`,page);const kpiWrap=q('#playerOverviewDashboard .kpis',page),mvpKpi=kpiWrap?qa('.kpi',kpiWrap)[1]:null;if(mvpKpi)mvpKpi.hidden=!showMvp;if(kpiWrap)kpiWrap.classList.toggle('without-mvp',!showMvp);const teamEl=q('.player-team',page);if(teamEl){teamEl.innerHTML='<span class="team-dot"></span>'+esc(p?.name||'—');const dot=q('.team-dot',teamEl);if(dot)dot.style.background=({BLUE:'var(--core-blue)',RED:'var(--core-red)',YELLOW:'var(--core-yellow)',GREEN:'var(--core-green)'}[p?.color]||'var(--theme-muted)')}setText('.profile-ranking .rank','#'+String(rank||1).padStart(2,'0'),page);const sel=q('#playerProfileSelect',page);if(sel){sel.innerHTML=state.actors.map(x=>`<option value="${esc(x.id)}" ${x.id===a.id?'selected':''}>${esc(x.name.toUpperCase())} · ${esc(actorParticipant(x)?.name||'')} ${x.id===state.userActorId?'· DU':''}${x.isBot?' · BOT':''}</option>`).join('')}setText('#playerSwitchContext',(a.id===state.userActorId?'EINGELOGGT · ':'PLAYER · ')+a.name.toUpperCase(),page);
 ...
 const awards=q('#v20AchievementGrid',page),mine=(state.awards||[]).filter(x=>x.actorId===a.id&&voteTypes.includes(String(x.type||'').toUpperCase()));if(awards){const awardLabel=voteTypes.join('/');awards.innerHTML=!showAwards?'':mine.length?mine.slice().reverse().map(x=>`<article class="v20-award-card"><small>GAME ${Number(x.gameIndex)+1} · ${esc(state.games?.[x.gameIndex]?.name||'GAME')}</small><h3>${esc(x.type)}</h3><p>${fmt(x.votes)} Stimmen · aktuelles Turnier</p></article>`).join(''):`<div class="v20-empty">NOCH KEIN ${esc(awardLabel)}-AWARD IN DIESEM TURNIER.</div>`}const ph=q('#v20PlayerHistory',page);if(ph)ph.innerHTML='<div class="v20-empty">FÜR DIESE DEMO WIRD NUR DAS AKTUELLE TURNIER ANGEZEIGT.</div>'}
function renderNews(){const page=q('#newsPage'),host=q('#v15NewsFeed',page);if(!page||!host)return;const visible=contextualNews(),rows=visible.length?visible:[{headline:'NOCH KEIN MATCH-REPORT.',body:'Sobald im aktuellen Turnier ein Ergebnis gespeichert wird, erzeugt SKIELSEN hier eine faktenbasierte Meldung.',type:'SYSTEM',at:nowLabel()}];host.innerHTML=rows.map(n=>`<article><small>${esc(n.type)} · ${esc(n.at)}</small><h2>${esc(n.headline)}</h2><p>${esc(n.body)}</p></article>`).join('')}
function renderJoker(){const page=q('#jokerPage');if(!page)return;if(!feature('feature.joker')){selectedJokerType=null;selectedJokerGameIndex=null;selectedJokerTarget=null;setText('#jokerFeedback','JOKER SIND IN DEN TURNIERSETTINGS DEAKTIVIERT.');return;}const p=userParticipant();setText('.utility-hero p',`${p?.name||'PLAYER'} · JOKER NUR AUF GEPLANTE SPIELE`,page);
 const pickOpponentTournamentEligible=(state.participants||[]).length===4;if(!pickOpponentTournamentEligible&&selectedJokerType==='PICK_OPPONENT'){selectedJokerType=null;selectedJokerTarget=null}
 const states=state.jokers?.[p?.id]||{};const availableTypes=['DOUBLE_POINTS',...(pickOpponentTournamentEligible?['PICK_OPPONENT']:[])].filter(type=>states[type]==='AVAILABLE');const effectiveType=selectedJokerType||(availableTypes.length===1?availableTypes[0]:null);
 ...
 qa('[data-joker-type]',page).forEach(b=>{const type=b.dataset.jokerType==='double_points'?'DOUBLE_POINTS':'PICK_OPPONENT',available=states[type]==='AVAILABLE',g=state.games?.[selectedJokerGameIndex],tournamentEligible=type!=='PICK_OPPONENT'||pickOpponentTournamentEligible,applicable=tournamentEligible&&(type!=='PICK_OPPONENT'||!g||pickOpponentAllowedForGame(g));b.hidden=!tournamentEligible;b.disabled=!available||!applicable;b.classList.toggle('selected',selectedJokerType===type&&tournamentEligible);const st=q(`[data-joker-state="${type}"]`,b);if(st)st.textContent=available?'VERFÜGBAR':'VERBRAUCHT'});
...
async function handleMatchDetailControlAction(btn){
 ...
 if(g.phase==='PLANNED'){
   ...
   try{const prepared=await prepareCurrentGame();if(prepared&&!feature('feature.joker')&&!feature('feature.betting')&&gameNow()?.phase==='ACTIVE'&&matchNow()?.status==='READY')return await startMatch();renderAll();saveSoon();return prepared!==false}
...
function bind(){if(bound)return;bound=true;
 ...
 if(!isAdmin()||!client||!runtime?.tournament_id)return;
 const resetParts=['laufende Matches','Ergebnisse','Game-Teilnahmen'];if(feature('feature.betting'))resetParts.push('Betting','Wallets');if(voteFeaturesEnabled())resetParts.push('Votes/Awards');if(feature('feature.joker'))resetParts.push('Joker-Submissions');resetParts.push('In-App-Sessions','Wortketten-Sessions','News','Runtime-Daten');
 if(!confirm('TURNIER KOMPLETT ZURÜCKSETZEN?\n\nGelöscht werden '+resetParts.join(', ')+'.\n\nGames, Reihenfolge, Teilnehmer, Teams und Feature-Settings bleiben erhalten.'))return;
 ...
 q('#gameControlStart')?.addEventListener('click',async e=>{const sel=gameControlSelection(),g=sel.g;if(!sel.isCurrent||e.currentTarget.disabled)return;if(g?.phase==='PLANNED'){if(!isAdmin())return;e.currentTarget.disabled=true;const prepared=await prepareCurrentGame();if(prepared&&!feature('feature.joker')&&!feature('feature.betting')&&gameNow()?.phase==='ACTIVE'&&matchNow()?.status==='READY')await startMatch();renderAll();saveSoon();return}e.currentTarget.disabled=true;await startMatch();renderGameControl()});
...
async function activate(rt){resetTransientTournamentPresentation();runtime=rt;client=window.skielsenDb?.client||null;if(!client){console.error('V15: no Supabase client');return}await hydrateThemeCatalog();await hydrateRuntimeThemeContract();try{const {data,error}=await client.from('participants').select('participant_id,participant_type,team_id,solo_member_id,identity_color,status,seed').eq('tournament_id',rt.tournament_id).eq('status','ACTIVE');if(!error&&Array.isArray(data))rt.participants=data;else if(error)console.warn('V15 participants hydrate',error)}catch(e){console.warn('V15 participants hydrate',e)}let loaded=null;if(rt.test_mode){try{const {data,error}=await client.rpc('get_tournament_test_runtime_state',{p_tournament_id:rt.tournament_id});if(!error&&data?.state)loaded=data.state}catch(e){console.warn(e)}}state=normalizeLoadedState(loaded,rt);if(!state.games?.length)state=defaultState(rt);await hydrateCanonicalTournamentResults(state,rt);ensureTournamentSchedule(state,state.participants);enforceGameLifecycleInvariant(state);selectedProfileActor=state.selectedProfileActorId||state.userActorId;document.getElementById('dbBootstrapOverlay')?.setAttribute('hidden','');document.body.classList.add('v15-tournament-active');setupHeader();bind();window.skielsenInApp?.start?.(runtime);if(feature('feature.joker'))await refreshServerJokerBoard(false);else serverJokerBoard=null;enforceGameLifecycleInvariant(state);if(feature('feature.joker'))startJokerBoardPolling();if(serverJokerBoard?.pending_pick){const pgi=(state.games||[]).findIndex(x=>x.tournament_game_id===serverJokerBoard.pending_pick.tournament_game_id);if(pgi===state.currentGameIndex)openPendingPickDialog(serverJokerBoard.pending_pick)}if(gameNow()?.phase==='PREPARING'&&gameNow()?.joker?.awaitingPick)startJokerResolutionPolling(gameNow());if(gameNow()?.phase==='ACTIVE'&&feature('feature.betting')){await refreshServerBettingState(gameNow(),matchNow());startBettingPolling();if(matchNow()?.status==='READY'&&!serverBettingState?.exists)openCurrentMarket(gameNow())}renderAll();syncJokerFeatureVisibility();const historyMode=rt.__historyMode||(window.skielsenHistory?.current()?.area==='workflow'?'push':'replace');show('home',historyMode);{const rg=gameNow();if(rg?.phase==='RESULTS'&&!rg?.awaitingInAppResultClose){if(rg.joker){rg.joker.revealed=true;rg.joker.revealAcknowledged=true}startPostGameVote(rg)}else if(['VOTING_MVP','VOTING_LVP'].includes(rg?.phase)){startPostGameVote(rg)}else if(rg?.phase==='AWARD_REVEAL'){const results=ensurePostGameVoteResults(rg);Object.values(results).forEach(r=>{if(r)r.revealAcknowledged=true});rg.postGameReveal={index:requiredPostGameVoteTypes().length,done:true};void advanceAfterVotes(true)}}saveSoon()}
window.skielsenV15Activate=activate;
window.skielsenV15={get state(){return state},get runtime(){return runtime},get gameControl(){return gameControlSelection()},get serverBettingState(){return serverBettingState},get serverVoteState(){return serverVoteState},render:renderAll,show,openMatchDetail,openGameControlForMatch,simulateOtherBets,simulateOtherVotes,prepareCurrentGame,startMatch,concludeCurrentMatch,gateComplete,bettingPendingCount,beginPostGameFlow,beginCanonicalMergedPostGameFlow,startPostGameVote,beginPostGameAwardReveal,refreshServerBettingState,refreshServerVoteState,leaveToAccountHome:leaveTournamentToAccountHome,restorePresentationForInApp};
})();
