...
if 'function syncMobileScrollIntent()' not in admin_command_text or "window.addEventListener('scroll',onWindowScroll,{passive:true})" not in admin_command_text: fail('Mobile Admin Command reagiert nicht direkt auf Scroll-Richtung')
...
if 'V15.1.91 · Mobile Admin Command responsive scroll collapse' not in main_css_text: fail('Mobile Admin Command Layout V15.1.91 fehlt')
...
if 'body.v15-tournament-active.admin-command-visible.admin-command-collapsed' not in main_css_text: fail('Mobile Admin Command passt Content-Offset beim Collapse nicht an')
