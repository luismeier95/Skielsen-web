...
if 'function setMobileCollapsed(next)' not in admin_command_text or "document.addEventListener('touchstart',onMobileTouchStart,{passive:true})" not in admin_command_text or "document.addEventListener('touchend',onMobileTouchEnd,{passive:true})" not in admin_command_text: fail('Mobile Admin Command Swipe-Collapse Runtime fehlt')
if 'setMobileCollapsed(dy<0);' not in admin_command_text or 'setMobileCollapsed(direction>0);' not in admin_command_text: fail('Mobile Admin Command Swipe-/Scroll-Richtung ist nicht definiert')
if 'function syncMobileScrollIntent()' not in admin_command_text or "window.addEventListener('scroll',onWindowScroll,{passive:true})" not in admin_command_text: fail('Mobile Admin Command reagiert nicht auf Scroll-Richtung')
if 'collapseScrollPending' not in admin_command_text or 'queueCollapseSync(true)' not in admin_command_text or 'if(inspectScroll)syncMobileScrollIntent()' not in admin_command_text: fail('Mobile Admin Command Scroll-Handling ist nicht per Animation Frame gebündelt')
if "document.addEventListener('touchmove',onMobileTouchMove,{passive:true})" not in admin_command_text or 'mobileDirectionTravel<10' not in admin_command_text: fail('Mobile Admin Command Fast-Swipe/Hysterese fehlt')
if 'V15.1.92 · Mobile Admin Command stable-geometry collapse' not in main_css_text: fail('Mobile Admin Command Layout V15.1.91 fehlt')
...
if 'body.v15-tournament-active #adminCommandBar.is-scroll-collapsed .admin-command-inner' not in main_css_text or 'height:44px!important;' not in main_css_text: fail('Mobile Admin Command kollabiert nicht auf eine Zeile')
if 'body.v15-tournament-active.admin-command-visible.admin-command-collapsed' not in main_css_text: fail('Mobile Admin Command Collapse-State fehlt')
if 'body.v15-tournament-active.admin-command-visible.admin-command-collapsed{\n    padding-bottom:270px!important;' not in main_css_text or 'body.v15-tournament-active.admin-command-visible.admin-command-collapsed.profile-page-active{\n    padding-bottom:328px!important;' not in main_css_text: fail('Mobile Admin Command verändert beim Collapse die Dokumenthöhe')
